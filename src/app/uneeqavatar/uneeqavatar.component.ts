...
export class UneeqavatarComponent implements OnInit, AfterViewInit {
  ...
  */
    ...
    Scene.prototype.onSceneMessage = (message: any) => {
      ...
      switch (name) {
        ...
        case 'conversationResult':
          ...
          const pie = body.output['context']['public-context']?.PIE
          const result = this.persona.conversationSend(pie, {}, { /* optionalArgs */ });
  ...
  avatarAnswerMessageHandler() {
    this.disableMicButton = false
    if (this.DescAnswer != '' && this.platformBrowser) {
      setTimeout(() => {
        ...
        if (this.DescAnswer === 'Let me think a bit.' || this.DescAnswer === 'Let me think') {
          ...
          if (localStorage.hasOwnProperty("learningId") && this.platformBrowser) {
  ...
  customWelcomeMessge() {
    ...
    if (savedDate) {
      ...
      if (this.scene.connectionState._connectionState['name'] == 'Disconnected') {
        ...
        personaInstance.conversationSend(text, {}, { /* optionalArgs */ });
        const savedDate2: any = (this.platformBrowser) ? localStorage.getItem('savedDate3') : '';
        if (savedDate2 == null) {
 ...
 */
  ...
  startScrolling() {
    if(this.platformBrowser){  /// start auto scrolling animation for learning modules
 ...
 */
  ...
  clickSendId1(event:any){
    if(this.platformBrowser) {
    if (event.keyCode == 13) {
      let click = document.getElementById('sendId1');
      click?.click()
    }
    }
  }
